    ...
    <div
      ...
      :class="{
        [$style.offsetTop]:
          isDeckbuilderActive &&
          (!isNotConjuration ||
            (card.phoenixborn && deckPhoenixborn && card.phoenixborn !== deckPhoenixborn.name)),
        shadow: isPopup,
      }"
    >
      <div
        class="bg-gray-light text-gray text-center relative border-b border-gray-light"
        :class="$style.imageHeight"
        aria-hidden="true"
      >
        <i class="text-2xl" :class="[typeIcon, $style.centerIcon]"></i>
        <img class="relative" width="300" height="75" alt=" " :src="cardImageURL" />
      </div>
      <div class="flow-root">
        ...
        <div class="px-2 py-px text-xs">
          <p class="m-0 font-bold text-lg">
            <router-link :to="linkTarget" class="text-black">{{
              card.name_ja !== undefined && card.name_ja !== '' && card.name_ja !== '\n'
                ? card.name_ja
                : card.name
            }}</router-link>
          ...
          <div v-if="card.text">
            ...
            <div class="leading-snug" :class="$style.effectText">
              <card-codes
                :content="
                  card.text_ja !== undefined && card.text_ja !== '' && card.text_ja !== '\n'
                    ? card.text_ja
                    : card.text
                "
          ...
          <div v-if="hasStats && !isPhoenixborn" class="text-center my-2 clear-fix">
            ...
            <span
              ...
              class="border border-gray-dark float-left px-1"
              >{{ card.copies }}</span
            >

            <span v-if="card.attack !== undefined" class="inline-block border border-red-light px-1"
              >Attack <strong>{{ card.attack }}</strong></span
            >
            ...
            <span
              ...
              class="inline-block border border-blue-dark px-1"
              >Recover <strong>{{ card.recover }}</strong></span
            >
...
export default {
  ...
  computed: {
    cardImageURL() {
      return this.showImage
        ? `${import.meta.env.VITE_CDN_URL}/images/slices/${this.card.stub}.jpg`
        : ''
    },
    typeIcon() {
  ...
  methods: {
    scrollCheck() {
      ...
      if (
        elementBounding.top <= window.innerHeight + LOAD_IMAGE_IF_WITHIN_PIXELS &&
        elementBounding.bottom >= -LOAD_IMAGE_IF_WITHIN_PIXELS
      ) {
        this.showImage = true
      } else {
