...
export function parseFormattedText(
  ...
  text = text.replace(
    /\[\[(\*?)([^\]]*?)((?:https?:\/\/|\b)[^\s/$.?#]+\.[^\s*]+?)\]\]|(https?:\/\/[^\s/$.?#]+\.[^\s*]+?(?=[.?)][^a-z]|!|\s|$))/gi,
    (_, isImage, text, url, standalone) => {
      ...
      if (isImage) {
        return `<a href="${textUrl}"${
          !internalLink ? ' rel="nofollow external"' : ""
        } target="_blank"><img class="object-contain" src="${textUrl}" alt=""></a>`;
      }
      return `<a href="${parsedUrl}"${!internalLink ? ' rel="nofollow"' : ""}>${
        text || textUrl
      }</a>`;
    }
  );
  ...
  text = text.replace(
    /\[\[(\*?)((?:[a-z ,-]|&#39;)+)(?::([a-z]+))?\]\]|( - )/gi,
    (input, isImage, primary, secondary, dash) => {
      if (dash) {
      ...
      if (diceList.indexOf(lowerPrimary) > -1) {
        ...
        return `<card-link :card="{name: '${primary.replace(
          "&#39;",
          "\\&#39;"
        )}', stub: '${lowerPrimary.replace(/ +/g, "-")}', is_legacy: ${isLegacy}}"></card-link>`;
      }
      return `<i class="phg-${lowerPrimary}-${secondary}" title="${primary}${
        secondary ? " " + secondary : ""
      }"><span class="alt-text">${input}</span></i>`;
    }
  );
  // Parse blockquotes
  text = text.replace(/(^> ?.+?)(?:(?=\n[^>\n])|$(?![^]))/gms, (match) => {
  ...
  text = text
    .replace(/<p>((?:<blockquote>)?<(?:u|o)l>)/g, "$1")
  ...
  text = text.replace(
    /<p>(<a class="inline-image".*?<\/a>(?=<\/p>|<br>))/g,
...
export function parseEffectText(text, isLegacy = false) {
  ...
  text = text
    .replace("<ul>", '<div class="inexhaustible-effects">')
